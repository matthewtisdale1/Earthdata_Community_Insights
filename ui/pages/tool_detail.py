...
from navigation import tools_link, goto_capability
...
def render() -> None:
    ...
    capabilities = api.get(f'/tools/{tool_code}/capabilities')

    breadcrumb('Earthdata Tools › Tool Details')
    page_header(tool['tool_name'], tool.get('description') or 'Earthdata tool or service')

    metrics = st.columns(6)
    metrics[0].metric('Capabilities', len(capabilities))
    metrics[1].metric('Sources', int(tool.get('source_count') or 0))
    metrics[2].metric('Artifacts', int(tool.get('artifact_count') or 0))
    metrics[3].metric('Issues', int(tool.get('issue_count') or 0))
    metrics[4].metric('Pull requests', int(tool.get('pull_request_count') or 0))
    metrics[5].metric('Releases', int(tool.get('release_count') or 0))

    overview_tab, capabilities_tab, sources_tab, artifacts_tab = st.tabs([
        'Overview', f'Capabilities ({len(capabilities)})', f'Sources ({len(sources)})', f'Artifacts ({len(recent_artifacts)})'
    ])
    ...
    with overview_tab:
        ...
        with right:
            with st.container(border=True):
                ...
                st.write(f"**Last synchronized:** {tool.get('last_synced_at') or 'Not yet synchronized'}")

    with capabilities_tab:
        if not capabilities:
            st.info('No capabilities are linked to this tool yet.')
        for capability in capabilities:
            with st.container(border=True):
                cols = st.columns([5, 1])
                with cols[0]:
                    st.markdown(f"### {capability['capability_name']}")
                    st.caption(f"{capability.get('category') or 'Uncategorized'} · {capability.get('support_level') or 'Support not assessed'}")
                    st.write(capability.get('description') or '')
                with cols[1]:
                    if st.button('Open', key=f"tool_cap_{capability['capability_code']}", use_container_width=True):
                        goto_capability(capability['capability_code'])
    ...
    with artifacts_tab:
        ...
        if not recent_artifacts:
            st.info('No implementation artifacts have been imported yet.')
        else:
